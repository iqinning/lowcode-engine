...
export function leafWrapper(Comp: types.IBaseRenderComponent, {
  ...
  class LeafHoc extends Component {
    ...
    makeUnitRender = () => {
      ...
      const nextProps = getProps(this.leaf?.export?.(TransformStage.Render), scope, Comp, componentInfo);
      // @todo 这里的边界 undefined 值如何处理？ @絮黎
      const children = getChildren(this.leaf?.export?.(TransformStage.Render), scope, Comp);
      const nextState = {
        nodeProps: nextProps,
